/**
 * Per-step reasoning effort (E24 T051).
 *
 * DeepSeek v4 models take a per-call `reasoning_effort` ('low' | 'medium' | 'high'). The
 * pipeline's contract-first steps — normalise_gap, plan_curriculum, audit_claims, diagnose
 * (and the independent-solutions verifier, which is also a contract step) — need direct,
 * compliant output, so they run at 'low' and their reasoning cannot eat the shared max_tokens
 * budget. The lesson generator benefits from deeper reasoning, so it runs at 'high' with the
 * larger output budget (32768) so reasoning AND content both fit.
 *
 ...
 * contract-first steps, 'high' on lesson generation, and the per-step budgets (plan 16384,
 * lesson 32768).
 */
 ...
 * normalise_gap, plan_curriculum, audit_claims and diagnose; verify_artefact is the same
 * family — independent solutions for a verification contract.)
 */
...
describe('per-step reasoning effort (E24 T051)', () => {
  ...
  it('runs lesson generation at high reasoning effort with the larger budget', async () => {
    const { context, calls } = buildRecordingContext();
    await compileEvalOne(context, 't051-lesson-high');
