...
describe('the durable worker loop (GAP-015)', () => {
  ...
  it('repairs a lesson the model shipped against the contract, inside the run', async () => {
    ...
    // On v4, max_tokens is shared between reasoning and content (T051): the plan runs at medium
    // reasoning effort (T052) so 16384 still fits it, while lessons run at 'high' and get 32768
    // so the reasoning trace AND the lesson JSON both fit.
    expect(observedTokens[0]).toBe(16384); // plan (medium effort)
    expect(observedTokens.slice(1).every((t) => t === 32768)).toBe(true); // lessons (high effort)
